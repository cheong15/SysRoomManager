...
//暂停
function handleJobPause(){
	$("div.group > a.link.pause").click(function() {
		if($(this).hasClass('disabled')){
			return false;
		}
		...
		if(len==0){
			$.ligerMessageBox.warn("提示信息","还没有选择,请选择一项进行暂停!");
			return;
		}
		else if(len>1){
			$.ligerMessageBox.warn("提示信息","已经选择了多项,请选择一项进行暂停!");
			return;
		}
...
function handleJobResume(){
	$("div.group > a.link.resume").click(function() {
		if($(this).hasClass('disabled')){
			return false;
		}
		...
		var len=aryId.length;
		if(len==0){
			$.ligerMessageBox.warn("提示信息","还没有选择,请选择一项进行恢复!");
			return;
		}
		else if(len>1){
			$.ligerMessageBox.warn("提示信息","已经选择了多项,请选择一项进行恢复!");
			return;
		}
		var name=aryId.attr("name");
		var value=aryId.val();
...
function handleJobRun(){
	$("div.group > a.link.run").click(function() {
		if($(this).hasClass('disabled')){
			return false;
		}
		...
		var len=aryId.length;
		if(len==0){
			$.ligerMessageBox.warn("提示信息","还没有选择,请选择一项进行运行!");
			return;
		}
		else if(len>1){
			$.ligerMessageBox.warn("提示信息","已经选择了多项,请选择一项进行运行!");
			return;
		}
		var name=aryId.attr("name");
		var value=aryId.val();
...
//刷新
function handleJobRefresh(){
	$("div.group > a.link.refresh").click(function() {
		if($(this).hasClass('disabled')){
			return false;
		}
