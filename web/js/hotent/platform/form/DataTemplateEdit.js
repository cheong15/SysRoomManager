...
DataTemplateEdit.prototype = {
	...
	initDisplaySetting : function() {
		...
		for (var i = 0, c; c = displayField[i++];) {
			...
			$("input[var='dicttype']", tr).attr("value", c.dicttype);
	...
	*
	*/
	getHtmlTdse : function(v, rightId, rightName, rightScript) {
		var aryTd = [
				'<select name="displayFieldRight" class="change_right" var="right"  >',
				'<option value="yes" '
						+ (v == 'yes' ? 'selected="selected"' : '')
						+ ' >是</option>',
				'<option value="no"'
						+ (v == 'no' ? 'selected="selected"' : '')
						+ '>否</option>'
				];
		return aryTd.join('');
	},
	...
	initImportField:function(){	
		var tbl = '#importFieldTbl';
		// 处理选择
		this.handChange(tbl);
		this.handClick(tbl);
		var tr = $($("#importFieldTemplate .table-detail")[0]).clone(true,
				true);
				
		var tabletr = $($("#importFieldTemplate .table-list")[0]).clone(true,
				true);		
		var tableVal = $("#exportField").val();
		//alert(tableVal);
		if ($.isEmpty(tableVal))
		...
		for (var l = 0, t; t = table[l++];) {
			...
			for (var i = 0, c; c = t.fields[i++];) {
				...
				$("input[var='ismain']", tr).attr("value", c.isMain);
				for (var j = 0, k; k = c.right[j++];) {
					var rightHtml = this.getHtmlTdse(k.type, k.id, k.name, k.script);
					if (k.s == 2) {
						$("[var='importRight']", tr).html(rightHtml);
					} 
				}
				sb.append(tr.html());
			}
