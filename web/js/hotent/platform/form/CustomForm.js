...
//			var subFileJsonList =obj.subFileJsonMap["subFileJsonList"];
		...
		initComdify:function(){
			$("div[type='custform']").delegate('[showtype]', 'blur',
					function() {
						var value = $(this).val();
						var json=null;
						try{
							var jsonStr=$(this).attr("showtype");
							json=eval('('+jsonStr+')');
	  ...
	  */
	  ...
	  initSubQuery:function(){
	  	$("select[selectquery]",$("div[type=subtable]")).each(function(){
		  	var me = $(this);
	  ...
	  //初始化子表选择器
		 subSelectorInit:function(){
	  		$('[ctlType="selector"]','.listTable').each(function(){
