...
FormUtil.initCommonDialog=function(){
	...
	$("a.extend",$(".formTable")).each(function(){      //只读时，历遍超链接中自定义对话框的按钮
		var extend=$(this);
		//区分主表中嵌套子表的情况
		var isSubTable = extend.closest('[type="subtable"]').length>0 ;
		//不处理子表情况，子表情况的放在CustomForm.js中处理了
		if(isSubTable) return true ;
						...
						if(self.val()!=""){
							...
							if(!selfClass==false){
								if(selfClass.indexOf("validError")>=0){
									isReturn = true ;
									$.ligerDialog.warn('请填写好--'+self.attr("lablename"),$lang.tip.msg);
									return false ;
								}
							}
						}
