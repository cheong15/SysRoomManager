...
function add(t,o){
	...
	$("[var=del]", table).click(function(e) {
		$.ligerDialog.confirm($lang_form.bpmFormTableTeam.delTeam, $lang.tip.msg, function(rtn) {
				if (rtn) {
...
function saveData(){
	...
	if(!rtn){
		alert($lang_form.bpmFormTableTeam.selectTeamField);
		return false;	
	}
	...
	$.post(url,{tableId:tableId,json:json},function(d){
		var result = eval("("+d+")");		
		if(result.success)
			$.ligerDialog.success(result.msg,$lang.tip.msg,function(){
				window.close();	
			});
		else
			$.ligerDialog.error(result.msg,$lang.tip.errorMsg);
	});
}