 ...
 *
 */
var controlList = [ {key : '1',value : $lang_form.editTable.control.text}, {key : '2',value : $lang_form.editTable.control.textarea},
                    {key : '10',value : $lang_form.editTable.control.ckeditor}, {key : '3',value : $lang_form.editTable.control.dictionary}, 
                    {key : '4',value : $lang_form.editTable.control.userSingle}, {key : '8',value : $lang_form.editTable.control.userMulti}, 
                    {key : '17',value : $lang_form.editTable.control.roleSingle},{key : '5',value : $lang_form.editTable.control.roleMulti}, 
                    {key : '18',value : $lang_form.editTable.control.orgSingle},{key : '6',value : $lang_form.editTable.control.orgMulti}, 
                    {key : '19',value : $lang_form.editTable.control.posSingle}, {key : '7',value : $lang_form.editTable.control.posMulti},
                    {key : '16',value : $lang_form.editTable.control.hidedomaid},{key : '9',value : $lang_form.editTable.control.attachement},
                    {key : '11',value : $lang_form.editTable.control.select},{key : '13',value : $lang_form.editTable.control.checkbox},
                    {key : '14',value : $lang_form.editTable.control.radio},{key : '12',value : $lang_form.editTable.control.officeControl},
                    {key : '15',value : $lang_form.editTable.control.datepicker},{key : '20',value : $lang_form.editTable.control.processInstance},
                    {key : '21',value : $lang_form.editTable.control.webSignControl},{key : '22',value : $lang_form.editTable.control.pictureShowControl}                     
                    ];
 ...
 * 值来源。
 */
var varFromList=[{key:0,value:$lang_form.editTable.varFrom.formInput},{key:1,value:$lang_form.editTable.varFrom.scriptDisplay},
                 {key:2,value:$lang_form.editTable.varFrom.scriptHide},{key:3,value:$lang_form.editTable.varFrom.serialNumber}];
...
function validTable(){
	var __valid__=$("#bpmTableForm").validate({
		...
		messages: {
			name:{
				required:$lang_form.editTable.tableNameRequired,
				maxlength:$lang_form.editTable.tableNameMaxlength
			},
			comment:{
				maxlength:$lang_form.editTable.tableDescMaxlength
			}
		}
	});
	return __valid__;
...
function validateField(){
	var __valid__ = $('#frmFields').validate({
		...
		messages : {
			fieldName : {
				required : $lang_form.editTable.fieldNameRequired
			},
			charLen : {
				required : $lang_form.editTable.charLenRequired,
				digits:$lang_form.editTable.digits
			},
			intLen : {
				required : $lang_form.editTable.intLenRequired,
				digits: $lang_form.editTable.digits
			},
			decimalLen : {
				required : $lang_form.editTable.decimalLenRequired ,
				digits: $lang_form.editTable.digits
			}
		}
	});
 ...
 */
function resetField(){
	$("#spanIntLen,#spanDecimalLen,#spanDecimalLen,#trDict,#trScript,#trScriptID,#trOption,#spanDateFormat,#trCondition,#showIsReference,#showidentity,#trUpLoad").hide();
	setControlByType("varchar");
	$("#fieldName,#fieldDesc").val("");
	$("#isRequired,#isFlowVar,#isQuery,#isReference,#isShowComdify,#coin,#isCurrentDate,#showCurUser,#showCurOrg,#showCurPos,#isWebSign").attr("checked",false);
...
function getField(){
	...
	if(field.fieldType==number_){
		...
		var showComdifyValue=$("#isShowComdify").attr("checked")?true:false;
		var json={};
		if(showCoin){
...
//	$("#ccisread").attr("checked",field.ccisread==1);
	...
	var curUser=null,curOrg=null,curPos=null;
	...
	hdlShowCurUserTrShow(ft,vf,ct,curUser);
	hdlShowCurOrgTrShow(ft,vf,ct,curOrg);
	hdlShowCurPosTrShow(ft,vf,ct,curPos);
	hdlIsWebSignShow(ft,vf,ct);
...
function bindExtTable(data,hasForm){
	...
	//绑外键
	bindPkField("relation",fieldList,table.relation,table.pkField);
	$("#pkField").bind('change',function(){
		var tempPkField=$(this).val();
		$("#relation").html("");
		bindPkField("relation",fieldList,table.relation,tempPkField);
	})
 ...
 */
function bindPkField(pkField,fieldList,defautValue,pkFieldParam){
	var obj=$("#" +pkField);
	for(var i=0;i<fieldList.length;i++){
		var field=fieldList[i];
		var option = optiontemplate.replaceAll('#value', field.fieldName).replace('#text', field.fieldDesc);
		if(pkFieldParam&&pkFieldParam==field.fieldName) continue;
		obj.append(option);
	}
...
function initCondition(selObj,fieldType){
	...
	switch(fieldType){
		case "varchar":
			selObj.append("<option value='='>"+$lang.operateType.gt+"</option>");
			selObj.append("<option value='like'>LIKE</option>");
			selObj.append("<option value='likeEnd'>LIKEEND</option>");
			break;
		case "number":
			selObj.append("<option value='='>"+$lang.operateType.gt+"</option>");
			selObj.append("<option value='>='>"+$lang.operateType.gtEq+"</option>");
			selObj.append("<option value='>'>"+$lang.operateType.gt+"</option>");
			selObj.append("<option value='<'>"+$lang.operateType.lt+"</option>");
			selObj.append("<option value='<='>"+$lang.operateType.ltEq+"</option>");
			break;
		case "date":
			selObj.append("<option value='='>"+$lang.operateType.gt+"</option>");
			selObj.append("<option value='between'>"+$lang.operateType.between+"</option>");
			break;	
	}
}
*/
...
function initSelValueFrom(selObj,val){
	switch(val){
		case "1":
			var html="<span id='selValInput'>"+$lang.operate.formInput+"</span>";
			selObj.html(html);
			break;
		case "2":
			...
			break;
		case "3":
			var html = "<a href='#' class='link var' title='"+$lang.operate.commonScript+"' onclick='selSelectScript(this)'>"+$lang.operate.commonScript+"</a></br>";
...
function hdlValueFromShow(fy,value){
	...
	case varchar_://文本
		...
		break;
	case clob_://大文本
		var varFromList=[{key:0,value:$lang_form.editTable.varFrom.formInput}];
		break;
	case date_://日期	
		var varFromList=[{key:0,value:$lang_form.editTable.varFrom.formInput}];
		break;
	case number_://数字
		var varFromList=[{key:0,value:$lang_form.editTable.varFrom.formInput},{key:1,value:$lang_form.editTable.varFrom.scriptDisplay},
		                 {key:2,value:$lang_form.editTable.varFrom.scriptHide}];
		break;
	}
	var objSelect=$('#valueFrom');
	objSelect.empty();
...
function hdlControlTypeShow(ft,vf,value){
	...
	case varchar_://文本
		...
		case '0'://表单输入
			controlTypeTr.show();
			controlList =[ {key : '1',value : $lang_form.editTable.control.text}, {key : '2',value : $lang_form.editTable.control.textarea},
                    {key : '10',value : $lang_form.editTable.control.ckeditor}, {key : '3',value : $lang_form.editTable.control.dictionary}, 
                    {key : '4',value : $lang_form.editTable.control.userSingle}, {key : '8',value : $lang_form.editTable.control.userMulti}, 
                    {key : '17',value : $lang_form.editTable.control.roleSingle},{key : '5',value : $lang_form.editTable.control.roleMulti}, 
                    {key : '18',value : $lang_form.editTable.control.orgSingle},{key : '6',value : $lang_form.editTable.control.orgMulti}, 
		...
		case '2'://脚本输入 (隐藏)
			controlTypeTr.show();
			controlList =  [ {key : '1',value : $lang_form.editTable.control.text}, {key : '2',value : $lang_form.editTable.control.textarea},
		                  	 {key : '3',value : $lang_form.editTable.control.dictionary}, 
		                    {key : '4',value : $lang_form.editTable.control.userSingle}, {key : '8',value : $lang_form.editTable.control.userMulti}, 
		                    {key : '17',value : $lang_form.editTable.control.roleSingle},{key : '5',value : $lang_form.editTable.control.roleMulti}, 
		                    {key : '18',value : $lang_form.editTable.control.orgSingle},{key : '6',value : $lang_form.editTable.control.orgMulti}, 
		                    {key : '19',value : $lang_form.editTable.control.posSingle}, {key : '7',value : $lang_form.editTable.control.posMulti},
		                    {key : '16',value : $lang_form.editTable.control.hidedomaid},{key : '9',value : $lang_form.editTable.control.attachement},
		                    {key : '11',value : $lang_form.editTable.control.select},{key : '13',value : $lang_form.editTable.control.checkbox},
		                    {key : '14',value : $lang_form.editTable.control.radio},
		                    {key : '15',value : $lang_form.editTable.control.datepicker},{key : '20',value : $lang_form.editTable.control.processInstance}];
			break;
		case '3'://流水号
			break;
	...
	case clob_://大文本
		...
		case '0'://表单输入
			controlTypeTr.show();
			controlList = [ {key : '2',value : $lang_form.editTable.control.textarea},
                   			{key : '10',value : $lang_form.editTable.control.ckeditor}];
			break;
		case '1'://脚本输入 (显示)
		case '2'://脚本输入 (隐藏)
		case '3'://流水号
	...
	case date_://日期
		...
		case '0'://表单输入
			controlTypeTr.show();
			controlList = [ {key : '1',value : $lang_form.editTable.control.text},{key : '15',value : $lang_form.editTable.control.datepicker},{key : '16',value : $lang_form.editTable.control.hidedomaid}];
			break;
		case '1'://脚本输入 (显示)
		case '2'://脚本输入 (隐藏)
		case '3'://流水号
	...
	case number_://数字
		switch(vf){
		case '0'://表单输入
			controlList[2] = {key : '11',value : $lang_form.editTable.control.select};
		case '1'://脚本输入 (显示)
		case '2'://脚本输入 (隐藏)
			controlTypeTr.show();
			controlList[0] = {key : '1',value : $lang_form.editTable.control.text};
			controlList[1] = {key : '16',value : $lang_form.editTable.control.hidedomaid};
			break;
		case '3'://流水号
			break;
		}
...
/**
 * 根据字段类型、值来源和控件类型，处理是否显示当前用户
 * @param fy 字段类型
 * @param vf 值来源
 * @param ct 控制类型
 */
function hdlShowCurUserTrShow(fy,vf,ct,value){
	vf=""+vf;
	ct=""+ct;
...
function hdlScriptTrShow(ft,vf,ct,value){
	...
	if(value!=undefined && value!=null && value!="" && tr.is(":visible")){
		$("#script").val(value);
	}
};
/**
...
function hdlIdentityTrShow(ft,vf,ct,identityAlias,identityName){
	...
	switch(vf){
		...
		case '3'://流水号
			tr.show();
			if(identityAlias){
				$("#identityAlias", tr).val(identityAlias);
				$("#identityName", tr).val(identityName);
			}
			break;
	}
};
 ...
 * @param ct 控制类型
 */
function hdlShowCurOrgTrShow(ft,vf,ct,value){
	vf=""+vf;
	ct=""+ct;
	var tr=$("#showCurOrgTr");
	tr.hide();
 ...
 * @param ct 控制类型
 */
function hdlShowCurPosTrShow(ft,vf,ct,value){
	vf=""+vf;
	ct=""+ct;
	var tr=$("#showCurPosTr");
	tr.hide();
