...
PictureShowPlugin={
			...
			for ( var i = 0; i < this.fileObjs.length; i++) {
				...
				if(right=="no"){
					...
					if(right=="w"||right=="b"){
						...
						/*$("a.link",div_rq).each(function(){      //只读时，历遍超链接
							var link=$(this);
		...
		upLoadPictureFile:function(t){
			...
			//IE浏览器是用：(OPEN方式打开的非模态窗口文件上传)  IE9在模态窗口文件上传有一点小问题所以...
			if(this.browserName=="MSIE"){
				DirectUploadDialog({uploadType:"pictureShow",fileFormates:"*.jpg;*.jpeg;*.bmp;*.png;*.gif;*.tiff;*.pcx;*.fpx;*.svg;*.psd",callback:function (curobj, attachs){
					//没有返回上传的图片相关信息时禁止执行
					...
					for(var i=0;i<attachs.length;i++){
						aryFileId.push(attachs[i].fileId);
						aryName.push(attachs[i].fileName);	
					}
