/**
 * 数据字典分类下拉框。 1.nodeKey: 对应数据分类表中的字典nodekey字段。
 * 
 * 2.下拉框类型： 1.dicComboBox： 表示普通的下拉框 2.dicCombo： 可为下拉框和树形下拉框。 3.dicComboTree：
 * 树形下拉框。 3.valueFieldID 字典对应的值。 4.isMultiSelect 是否允许多选。 5.value 下拉框选择的值。
 * 6.treeLeafOnly 如果是树形下拉框，只选择叶子节点。
 * 
 * 7.height 下拉框的高度。 <input class="dicComboBox" nodeKey="xueli"
 * valueFieldID="BoxMultiId" name="BoxMultiName" isMultiSelect="true"
 * width="200" value="博士"/>
 */
$(function() {
	var ctx = __ctx;
	...
	function readProp(dicCombo) {
		...
		var prop = {
			...
			canreset : $(dicCombo).attr("canreset"),
		...
		// 宽度
		if (isObjNull(prop.width))
			prop.width = isObjNull($(dicCombo).width()) ? 120 : $(dicCombo)
					.width() - 15;
		// 高度
		if (isObjNull(prop.height))
			prop.height = isObjNull($(dicCombo).height())
					&& $(dicCombo).height() < 50 ? 100 : $(dicCombo).height();
		// 字段值的ID
		if (isObjNull(prop.valueFieldID)) {
			// 表单字段命名为 m:表名:字段名称，在提交数据的时候程序会检查表单名称为m:开头的字段，这个id是不提交的，所以替换掉。
			prop.valueFieldID = $(dicCombo).attr("name").replace("m:", "")
					.replace("s:", "")
					+ "_id";
		}
		// 是否多选
		if (isObjNull(prop.isMultiSelect))
										...
										.ligerComboBox(
												{
													...
													onSelected : function(
														...
														/*
														 * if
														 * (this._toggleSelectBox) {
														 * this
														 * ._toggleSelectBox(true); }
														 * else if
														 * (ligerComboObj &&
														 * ligerComboObj._toggleSelectBox) {
														 * ligerComboObj
														 * ._toggleSelectBox(true); }
														 */
														if (prop.onSelected) {
															eval(prop.onSelected
																	+ '.call(this,newval)');
														}
													}
												});
