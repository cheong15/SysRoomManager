				...
				.each(
						function() {
							...
							if (thisWidgetSettings.removable) {
								var ifHasRemove = $(settings.handleSelector,
										this).find("a.remove");
								if (ifHasRemove.length == 0) {
							...
							if (thisWidgetSettings.collapsible) {
								var ifHasColl = $(settings.handleSelector, this)
										.find("a.collapse");
								if (ifHasColl.length == 0) {
									$(
