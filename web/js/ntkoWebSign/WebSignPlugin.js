			...
			if(this.fileObj.length>0){
				...
				if(right=="no"){
					...
					if(right=="r"||right=="rp"){   //只读时，没有按钮操作
						this.webSignObj.loadWebSign(divId,"ntkoocx"+divId,fileId,{imgStyle:'display: none;',buttonStyle:"display: none;"});
					}else{   //可编辑 有按钮操作
						this.webSignObj.loadWebSign(divId,"ntkoocx"+divId,fileId,{imgStyle:'display: none;',buttonStyle:"display: block;"});
					}
