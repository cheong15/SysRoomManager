...
NtkoWebSign=function(){
	{
		...
		this.width="0";     //隐藏
		this.checkForm="";
		this.configStyle={imgStyle:'display: none;',buttonStyle:"display: none;"};
	};
...
/*		tags+='<div id="webSignButtonSeal" style="'+this.configStyle.buttonStyle+'" >';
	...
	this.AddSecHandSign=function()
	{
		...
		this.webSignObj.AddSecHandSign(secSignObj,'系统管理员0',0,false,false,true,true,null,true);
	};
	...
	this.AddSecHandSignNoPrompt=function()
	{
		...
		if( strxy!="" && (strxy.split("&").length==2)){
			...
			//	secSignObj.HandSignPenColor = 0; // 此数值计算方式为：假设需要设定颜色值RGB,则value = (blue * 65536) + (green * 256) + red
				...
				this.webSignObj.AddSecHandSign(secSignObj,'系统管理员0',2,false,false,true,false,null,false,80);
			}
		}		
	};
	...
	this.AddSecHandSignAdjust=function()
	{
		var secSignObj = this.webSignObj.AddSecSignOcx("SecHandSignID",20,20);
		secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
		this.webSignObj.AddSecHandSign(secSignObj,'系统管理员0',0,false,false,true,true,null,true,50);
	};
	...
	this.AddSecKeyBoardComment=function()
	{
		...
		secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
		this.webSignObj.AddSecKeyBoardComment(secSignObj,'系统管理员0',0,false,false,true,false);
	};
	...
	this.AddSecSignFromLocal=function(isAddComment)
	{
		...
		secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
		this.webSignObj.AddSecSignFromLocal(secSignObj,'系统管理员1','',true,1,false,false,true,true,null,isAddComment);
	};
	...
	this.AddSecSignFromURL=function()
	{
		...
		this.webSignObj.AddSecSignFromURL(secSignObj,'系统管理员2',
	...
	this.AddSecSignFromService=function(){
		...
		//	alert("X:"+arrys[0]+"   Y:"+arrys[1]);			
			...
			try{
				...
				secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
				//secSignObj.PositionTagId = "JiaFanSpan";//定义印章加盖的位置 JiaFanSpan 一般为DIV
				this.webSignObj.AddSecSignFromURL(secSignObj,'系统管理员',signUrl,2,false,false,true,false,"",false);
			}catch(err){
	...
	this.AddSecSignFromEkey=function()
	{
		if(this.webSignObj!=null){
			...
			secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
			this.webSignObj.AddSecSignFromEkey(secSignObj,'系统管理员1',2,false,false,true,true);
		}
	};
	...
	this.AddSecSignFromEkeyWithSignIndex=function()
	{
		if(this.webSignObj!=null){
			...
			var secSignObj = this.webSignObj.AddSecSignOcx("SecSignFromEkeyID",20,20);
			secSignObj.WebSignInfo = this.checkPrintForm();          //要Web签章验证的信息;
			this.webSignObj.AddSecSignFromEkey(secSignObj,'系统管理员1',2,false,false,true,true,null,true,1/*SignIndex*/);
		}
	};
