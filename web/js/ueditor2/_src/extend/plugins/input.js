			...
			execCommand : function(cmdName,data,old) {
				if(data){					
					if(old){
						...
						old.setAttribute("external",getExternal(data));
						old.firstChild.setAttribute("style",previewstyle);
					}
					else{
...
//					html+='<tr><th>下拉框选项</th><td colspan="3"><span style="color:green;">每行对应一个选项<br /></span><textarea eid="options" cols="50" rows="5"></textarea></td></tr>';
	...
	me.commands['personpicker'] = {
			execCommand : function(cmdName,data,old) {
				if(data){	
					...
					me.curInputHtml=null;
					var html='<tr><th>按钮文本</th><td><input eid="buttoncontent" type="text" value="选择" /></td><th>其他设定:</th><td><label><input eid="singleselect" type="checkbox" />单选</label><label><input eid="showCurUser" type="checkbox" />显示当前用户</label></td></tr>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}
			},
	...
	function getContentHtml(html){
		...
		content.push('<tr class="valuefrom12 hidden"><th>脚本(显示):</th><td colspan="3"><div><a href="#" onclick="addScript(this)" class="link var" title="常用脚本">常用脚本</a></div><textarea eid="content" prenode="valueFrom" cols="50" rows="5"></textarea></td></tr>');
