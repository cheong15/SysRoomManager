...
function delRows(rowId){
	...
	if(cks.length==0){
		$.ligerDialog.warn($lang.del.selectRecord,"提示信息");
		return ;
	}
...
function selectCmp(obj){
	...
	switch(assignType){
		...
		//表单变量
		case "formVar":
			showFormVar({callback:callback,nodeUserId:nodeUserId,defId:defId,cmpIds:cmpIds.val()});
			break;
	}
}
...
function assignTypeChange(obj){
	...
	if(obj.value=="startUser"||obj.value=="sameDepart"||obj.value=="directLeader" || obj.value=="prevUserOrgLeader" 
