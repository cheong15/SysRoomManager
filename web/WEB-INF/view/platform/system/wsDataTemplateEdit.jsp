...
<script type="text/javascript">
	...
	function aliasChange(){
		...
		$.post('${ctx}/platform/system/wsDataTemplate/getWsDocumentById.ht',{'setId':setId},function(doc){
			if(doc){
				...
				$.ligerDialog.error($lang.tip.errorMsg,$lang.tip.errorMsg);
			}
		});
	};
	...
	function doExecute(){
		...
		if(!rtn) {
			$.ligerDialog.warn("请填写参数信息！",$lang.tip.warn);
			return false ;
		}
		...
		$.post(url,{setId:setId,json:json},function(d){
			...
			if(data.result){
				...
				$.ligerDialog.error($lang.tip.errorMsg + data.message);
			}
		});
	};
	function doPrase(){
		...
		if(returnXML.trim().length<=2){
			$.ligerDialog.warn('返回的XML不能为空！',$lang.tip.warn);
			return false ;
		}
		if(parseScript.trim().length<=2){
			$.ligerDialog.warn('解析脚本不能为空！',$lang.tip.warn);
			return false ;
		}
		var requestObj = {};
	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">编辑WEB服务数据模板</span>
			</div>
		...
		<form id="wsTemplateDataEdit" method="post" action="save.ht">
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0">				 
				...
				<tr>
					<td style="line-height: 30px;"><a class="link detail" href="#"><span></span>解析</a></td>
				</tr>
				<tr>
					<th width="20%">模板:</th>
					<td>
