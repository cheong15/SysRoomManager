...
<script type="text/javascript">
	...
	function save(){
		if(!validRequired()){
			$.ligerDialog.warn($lang_bpm.messageCopyEdit.inputTitle,$lang.tip.msg);
			return;
		}
		...
		if(!_actDefId){
			$.ligerDialog.warn($lang_bpm.messageCopyEdit.notDef,$lang.tip.msg);
			return;
		}		
		...
		$.post(url,{data:json,actDefId:_actDefId,conditionId:_conditionId,messageSetType:_messageSetType},function(d){
			...
			if(r.result){
				$.ligerDialog.success(r.message,$lang.tip.msg,function(){
					window.opener.location.reload();
					window.close();
				});
			}
			else
				$.ligerDialog.error(r.message,$lang.tip.msg);
		});
	};
</script>
</head>
...
<div class="panel">
	<div class="panel-top">
		...
		<div class="panel-toolbar">
			<div class="toolBar">
				<div class="group"><a class="link save" onclick="save()" href="#">保存</a></div>
		...
		<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
			...
			<tr id="inner-tr" class="hidden">
				...
				<td>
					...
					<div>
						<a href="#"  class="link var" title="选择模板" onclick="slectTemplate('innerContent',false)">选择模板</a>
					</div>
			...
			<tr id="mail-tr" class="hidden">
				...
				<td>
					<input type="text" name="subject" value="${sysTemplate.title}" class="inputText" style="width:325px !important;margin:8px 0 18px 0;"/>
					<div>
						<a href="#"  class="link var" title="选择模板" onclick="slectTemplate('mailContent',false)">选择模板</a>
					</div>
			...
			<tr id="sms-tr" class="hidden">
				...
				<td>
					<div>
						<a href="#"  class="link var" title="选择模板" onclick="slectTemplate('smsContent',true)">选择模板</a>
					</div>
