...
<script type="text/javascript">
	...
	function initSearchForm(){
		...
		for(var i=0;i<params.length;i++){
			...
			$(':input',li).attr('name',field);
			if(defaultType=='0'){//固定值
				$(':input',li).attr('value',obj.defaultValue);
			}else if(defaultType=='1'){//控件
				$(':input',li).attr('value',paramMap[field]);
				if(paraCt=="0"){//自定义对话框
	...
	function getInput(obj){
		...
		var getSelector = function(str){
			...
			var control = $("#"+str).clone(true,true).removeAttr("id");
		 	$("input[type='text']",control).attr("name",value).attr("disabled","true");
		 	$("input[type='text']",control).hide();
		 	$("input[type='hidden']",control).attr("name",valueName).attr("disabled","true").attr('onchange','onHideFileChange(this)');
...
<div class="panel-top">
	<div class="panel-toolbar">
		<div class="toolBar area-export">
			<div class="group exportlinks" title="导出PDF" onclick="save('pdf')">
				<div class="pdf" style="width:40px;">PDF</div>
			</div>
			...
			<div class="group leftGroup">
				<div class="l-bar-button l-bar-btnnext">
					<a href="#" onclick="next('')" title="下一页"> <span></span>
					</a>
				</div>
			...
			<div class="group leftGroup">
				<div class="l-bar-button l-bar-btnfirst">
					<a href="#" onclick="first('')" title="首页"> <span class=""></span>
					</a>
				</div>
