...
<#--表体-->
				...
				<tr class="<#noparse><#if data_index % 2 == 0>odd</#if><#if data_index % 2 == 1>even</#if></#noparse>">
					...
					<#if (editable && pkcols?? && pkcols?size>0)>
					<td>
						<a href="#" onclick="openLinkDialog({scope:this,isFull:true})" action="${editBaseURL}?&__displayId__=<#noparse>${sysTableManage.id}</#noparse><#list pkcols as col>&__pk__${col.name}=<#noparse>${data.</#noparse>${col.name}<#noparse>}</#noparse></#list>" class="link edit">
							编辑
						</a>
