...
public abstract class BasePluginType implements PluginTypeInterface{
   ...
   */
	...
	protected List<JarFileAnnotationPlugin> findAnnotatedClassFiles(String annotationClassName) {
		...
		List<PluginFolderInterface> pluginFolders = getPluginFolders();
		if (pluginFolders != null && pluginFolders.size() > 0) {
			for (PluginFolderInterface pluginFolderInterface : pluginFolders) {
				String folderName = pluginFolderInterface.getFolder();
				if ("plugins".equals(folderName )) {
					int jarFileSize = 0;
					try {
						FileObject[] findJarFiles = pluginFolderInterface.findJarFiles();
						if (findJarFiles != null) {
							jarFileSize = findJarFiles.length;
						}
					} catch (KettleFileException e) {
						e.printStackTrace();
					}
					pluginFolders.remove(pluginFolderInterface);
					LogChannel.GENERAL.logDetailed("正在移除特定判断的文件夹  " + folderName + " ! 其中包含 " + jarFileSize + " 个jar文件!");
					break;
				}
			}
		}
