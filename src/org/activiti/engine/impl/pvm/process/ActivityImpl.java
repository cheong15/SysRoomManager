 ...
 * @author Tom Baeyens
 */
public class ActivityImpl extends ScopeImpl implements PvmActivity, HasDIBounds ,Cloneable{
  ...
  public void setProcessDefinition(ProcessDefinitionEntity ent){
	  this.processDefinition=ent;
  }
  

  
  public Object clone() {  
	  ActivityImpl ent=null;
	 try{
		 ent=(ActivityImpl)super.clone();
		 List<TransitionImpl> outGoingList=new ArrayList<TransitionImpl>();
		 Map<String,TransitionImpl> outGoingMap=new HashMap<String, TransitionImpl>();
		 ent.namedOutgoingTransitions=outGoingMap;
		 ent.setOutgoingTransitions(outGoingList);
		 for(TransitionImpl trans:outgoingTransitions){
			 outGoingMap.put(trans.id, trans);
			
			 outGoingList.add(trans);
		 }
