...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution, Execution, PvmExecution, ProcessInstance, InterpretableExecution, PersistentObject {
  ...
  /** 
   * 指示当前执行是否四海一个活动的excution。
   * indicates if this execution represents an active path of execution.
   * excution 变成非激活的情况为一下几种情况。
   * Executions are made inactive in the following situations:
   * <ul>
   *   <li>an execution enters a nested scope ，一个excution 进入到一个嵌套的范围。</li>
   *   <li>an execution is split up into multiple concurrent executions, then the parent is made inactive.
   *   	excution 分解成多个并发的执行。
   *   </li>
   *   <li>an execution has arrived in a parallel gateway or join and that join has not yet activated/fired.
   *   		一个执行到达一个并行网关，或者聚合并且聚合还没有激活
   *   </li>
   *   <li>an execution is ended.
   *   	excution 结束。
   *   </li>
   * </ul>*/ 
