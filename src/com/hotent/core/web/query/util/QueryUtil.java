...
public class QueryUtil {
	...
	/**
	 * 将将符合BPM_DATA_TEMPLATE表的FILTERFIELD字段的字符串内容的condition部分转成FilterJsonStruct集合
	 * 这样是为了重用。
	 * 
	 * @param condition
	 * @return
	...
	public static List<FieldLogic> buildFieldLogics(
		...
		CommonVar.setCommonVar(params, ContextUtil.getCurrentUserId(),
				ContextUtil.getCurrentOrgId());
		List<FieldLogic> fieldLogics = new ArrayList<FieldLogic>();
		for (Iterator<FilterJsonStruct> iterator = filterJsonStructs.iterator(); iterator
				.hasNext();) {
			...
			if (filterJsonStruct.getBranch()) {// 组合条件
				List<FieldLogic> groupLogics = buildFieldLogics(filterJsonStruct
						.getSub());
	...
	public static Set<String> getTables(List<FieldLogic> fieldLogics) {
		...
		if (fieldLogics == null || fieldLogics.size() == 0)
			return tableSet;
		for (Iterator<FieldLogic> iterator = fieldLogics.iterator(); iterator
				.hasNext();) {
	...
	public static List<FieldSort> buildFieldSorts(String sortJson) {
		...
		List<FieldSort> fieldSorts = com.alibaba.fastjson.JSONArray.parseArray(
				sortJson, FieldSort.class);
		return fieldSorts;
	}
		...
		while (params.hasMoreElements()) {
			...
			if (values.length > 0 && StringUtils.isNotEmpty(values[0])) {
				if (key.startsWith("Q_")) {// 只针对查询条件的
					FilterJsonStruct filterJsonStruct = setFilterJsonStruct(
							key, values, mainTableName, tableFieldMap);
			...
			if (StringUtil.isNotEmpty(value)) {
				try {
					...
					f = getFilterJsonStruct(mainTableName, tableField,
							paraName, relation);
					if (relation.equals(QueryConstants.SQL_RELATION_IN))
						f.setScript(f.getScript().replace(
								relation + " :" + paraName,
								" IN ('" + value.replace(";", "','") + "')"));
				} catch (Exception e) {
	...
	private static String getQueryRelation(String type) {
		...
		if ("S".equals(type) || "L".equals(type) || "N".equals(type)
				|| "DB".equals(type) || "BD".equals(type) || "FT".equals(type)
				|| "SN".equals(type)) {
			relation = QueryConstants.SQL_RELATION_EQUAL;
		}
		// 字符串 模糊查询
		else if ("SL".equals(type) || "SLR".equals(type) || "SLL".equals(type)
				|| "SUPL".equals(type) || "SUPLR".equals(type)
				|| "SUPLL".equals(type) || "SLOL".equals(type)
				|| "SLOLR".equals(type) || "SLOLL".equals(type)) {
			relation = QueryConstants.SQL_RELATION_LIKE;
		}// 字符串 IN
		else if ("SIN".equals(type)) {
			relation = QueryConstants.SQL_RELATION_IN;
		}
		// date begin
	...
	private static FieldSort getPostFieldSort(QueryFilter queryFilter,
		...
		return new FieldSort(tableName, String.valueOf(orderField),
	...
	public static String getQuerySQL(BusQueryRule busQueryRule,
		...
		// 获得过滤的条件
		String filterKey = RequestUtil.getString(request,
				QueryConstants.FILTER_KEY);
		if (BeanUtils.isNotEmpty(busQueryFilter)) {
			...
			fieldSort = getPostFieldSort(queryFilter,
		...
		if (JSONUtil.isNotEmpty(filterJson)) {
			...
			if ("2".equals(type)) {
				...
				CommonVar.setCommonVar(params, userId, curOrgId);
	...
	private static FieldSort getQuerySort(BusQueryFilter busQueryFilter,
		...
		return new FieldSort(tableEntity.getTableName(),
				String.valueOf(orderField), String.valueOf(orderSeq));
	}
	...
	public static List<?> getPageList(BusQueryRule busQueryRule,
		...
		if (isQuery == QueryConstants.IS_QUERY_YES) {
			...
			// [CUR_USER]
			querySQL = querySQL.replace("[CUR_USER]", ContextUtil
					.getCurrentUserId().toString());
			// [CUR_ORG]
			Long orgId = ContextUtil.getCurrentOrgId();
			if (orgId != null && orgId > 0) {
				querySQL = querySQL.replace("[CUR_ORG]", orgId.toString());
			}
			...
			if(queryFilter.getPageBean() != null && busQueryRule.getNeedPage().intValue() == 1) {
				...
				int oldPageSize = RequestUtil.getInt(request, tableIdCode
						+ "oz", -1);
				if (oldPageSize < 0)
					pageSize = (page.getPageSize() != busQueryRule
							.getPageSize()) ? busQueryRule.getPageSize() : page
							.getPageSize();
				PageBean pageBean = new PageBean(currentPage, pageSize);
				queryFilter.setPageBean(pageBean);
				list = jdbcHelper.getPage(querySQL, paraMap, pageBean);
			} else {
				list = jdbcHelper.queryForList(querySQL, paraMap);
			}
		}
		// 设置回请求对象，避免前台页面报错
		queryFilter.setForWeb();
	...
	public static TableEntity getTableEntity(String displayTagId,
		...
		TableEntity tableEntity = SearchCache.getDisplayTagIdMap().get(
				displayTagId);
		if (tableEntity != null)
			return tableEntity;
	...
	public static BusQueryRule getBusQueryRule(String displayTagId,
		...
		Long filterFlag = RequestUtil.getLong(request,
				QueryConstants.FILTER_FLAG, null);
		// 当前用户的权限
		...
		BusQuerySetting busQuerySetting = busQuerySettingService
				.getByTableNameUserId(tableName, ContextUtil.getCurrentUserId());
		permission = getPermissionSetting(permission, busQuerySetting);
		// 过滤列表
		List<Filter> filterList = QueryUtil.getFilter(busQueryRule, rightMap,
				filterKey);
	...
	private static Map<String, Boolean> getPermissionSetting(
		...
		Map<String, Boolean> right = getDisplayFieldShow(busQuerySetting
				.getDisplayField());
	...
	private static Map<String, Boolean> getPermission(int type,
	 ...
	 * @return
	 */
	private static Map<String, Boolean> getPermissionMap(int type,
		...
		for (Object obj : jsonAry) {
			...
			for (Object right : rights) {
				...
				if (s.intValue() == type)
					map.put(var, hasRight(rightJson, rightMap));
			}
		}
		return map;
	}
	...
	public static Map<String, Object> getRightMap() {
		...
		SysUserOrgDao sysUserOrgDao = (SysUserOrgDao) AppUtil
				.getBean(SysUserOrgDao.class);
	...
	public static String getFilterKey(BusQueryRule busQueryRule,
		...
		if (StringUtils.isNotEmpty(filterKey)
			...
			return filterKey;
		JSONArray jsonArray = JSONArray.fromObject(busQueryRule
				.getFilterField());
		if (JSONUtil.isEmpty(jsonArray))
			return filterKey;
