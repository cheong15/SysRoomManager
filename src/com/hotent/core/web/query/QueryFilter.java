...
public class QueryFilter 
{
	 ...
	 * Displaytag的tableId。
	 * 对应displaytag的id属性
	 * 通过tableId可以区分同一个页面有多个Display table
	 */
	...
	/**
	 * add by Liangyy
	 * 构建用于JsonRequest请求查询参数、分页信息
	 * @param request
	 * @param object
	 */
	public QueryFilter(HttpServletRequest request, Object object) {
		this(request, object, true);
	}
	 ...
	 * @param needPage
	 */
	public QueryFilter(HttpServletRequest request, Object object, boolean needPage) {
		...
		try {
			...
			if (needPage) {
				...
				if (pageInfo == null) {
					pageInfo = new PageInfo(1, PageBean.DEFAULT_PAGE_SIZE);
				}

				int page = (pageInfo.getCurrentPage() == 0 ? 1 : pageInfo.getCurrentPage());
				int pageSize = (pageInfo.getPageSize() == 0 ? PageBean.DEFAULT_PAGE_SIZE : pageInfo.getPageSize());
	 ...
	 * @param tableId tableId 对应diaplaytag的 id属性
