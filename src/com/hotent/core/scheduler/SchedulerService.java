...
//	@Resource
	...
	public ResultMessage addJob(String jobName,Class cls,Map parameterMap,String description, String groupName)  {
		if(scheduler==null ) return new ResultMessage(ResultMessage.Fail,"scheduler 没有配置!"); 
	...
	/**
	 * <p>Description: add by zhangyuqing<p>
	 * @param jobName
	 * @param className
	 * @param parameterMap
	 * @param description
	 ...
	 * @throws SchedulerException
	 */
	public ResultMessage  addJob(String jobName,String className,Map parameterMap,String description, String groupName) throws SchedulerException{
		if(scheduler==null ) return new ResultMessage(ResultMessage.Fail,"scheduler 没有配置!"); ;
		...
		try{
			resultMsg = addJob(jobName,cls,parameterMap,description, groupName);
			return resultMsg;
		}
		catch (Exception e) {
			resultMsg=new ResultMessage(ResultMessage.Fail,e.getMessage());
	...
	/**
	 * <p>Description: 根据JOB分组名称获取JOB列表<p>
	 * @param groupName
	 * @return
	 * @throws SchedulerException
	 */
	public List<QrtzTriggers> getJobLisyByGroupName(String groupName) throws SchedulerException{
		if(scheduler==null ){
			return new ArrayList<QrtzTriggers>();
		}
		...
		for(JobKey jobKey : set) {
			...
			if (detail != null) {
				...
				if (jobDataMap != null) {
			        ...
			        qrtzTriggers.setErrorNoticeUserId(jobDataMap.getString("errorNoticeUserId"));
			        qrtzTriggers.setErrorNoticeUserName(jobDataMap.getString("errorNoticeUserName"));
			        qrtzTriggers.setDescription(detail.getDescription());
				}
			}
			
			List<? extends Trigger> triggersList = scheduler.getTriggersOfJob(jobKey);
			if (triggersList != null && triggersList.size() > 0) {
				Trigger trigger = triggersList.get(0);
				TriggerState triggerState = scheduler.getTriggerState(trigger
						.getKey());
				if (triggerState != null) {
					qrtzTriggers.setTriggerState(triggerState.ordinal());
					qrtzTriggers.setTriggerStateName(triggerState.name());
				}
				qrtzTriggers.setNextFireTime(StringUtil.DateToString(
						trigger.getNextFireTime(), "yyyy-MM-dd HH:mm:ss"));
				qrtzTriggers.setPrevFireTime(StringUtil.DateToString(
						trigger.getPreviousFireTime(), "yyyy-MM-dd HH:mm:ss"));
				qrtzTriggers.setStartDate(StringUtil.DateToString(
						trigger.getStartTime(), "yyyy-MM-dd HH:mm:ss"));
				qrtzTriggers.setEndDate(StringUtil.DateToString(
						trigger.getEndTime(), "yyyy-MM-dd HH:mm:ss"));
				
				if ((trigger instanceof CronTrigger)) {
					qrtzTriggers.setCronString(((CronTrigger) trigger)
							.getCronExpression());
				} else if ((trigger instanceof SimpleTrigger)) {
					qrtzTriggers.setRepeatInterval(((SimpleTrigger) trigger)
							.getRepeatInterval());
					qrtzTriggers.setRepeatCount(((SimpleTrigger) trigger)
							.getRepeatCount());
				}
			}else{
				qrtzTriggers.setTriggerState(-1);
			}
	...
	/**
	 * <p>Description: 删除任务　add by zhangyuqing<p>
	 * @param jobName
	 * @param groupName
	 * @throws SchedulerException
	 */
	public void delJob(String jobName, String groupName)throws SchedulerException {
		if(scheduler==null ){
			return ;
		}
	 ...
	 * @throws SchedulerException
	 */
	public void addTrigger(String jobName, String groupName, String triggerName, TriggerBuilder<Trigger> triggerBuild) throws SchedulerException{
		if(scheduler==null ){
			return ;
		}
		JobKey jobKey=new JobKey(jobName, groupName);
		triggerBuild.withIdentity(triggerName, groupName);
		triggerBuild.forJob(jobKey);
		Trigger trigger = triggerBuild.build();
		scheduler.scheduleJob(trigger);
	}

	public void delTrigger(String triggerName, String kETTLE_GROUP_NAME) throws SchedulerException{
		TriggerKey triggerKey = new TriggerKey(triggerName, schedGroup);
		scheduler.resumeTrigger(triggerKey );
	}
	
	/**
	 * <p>Description: 根据JOB名称和组名查找JOB详细<p>
	 * @param jobName
	 * @param groupName
	 * @return
	 */
	public JobDetail getJobDetailByJobNameAndGroupName(String jobName, String groupName){
	...
	/**
	 * <p>Description: 根据JOB名称和组名暂停JOB<p>
	 * @param jobName
	 * @param groupName
	 * @throws SchedulerException
	 */
	...
	/**
	 * <p>Description: 根据JOB名称和组名恢复JOB<p>
	 * @param jobName
	 * @param groupName
	 * @throws SchedulerException
	 */
