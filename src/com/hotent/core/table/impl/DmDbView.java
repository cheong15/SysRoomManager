...
public class DmDbView extends BaseDbView implements IDbView {
	...
	public List<String> getViews(String viewName) throws SQLException {
		...
		if(StringUtil.isNotEmpty(viewName)){
			sql+=" and lower(view_name) like '"+viewName.toLowerCase()+"%'";
		}
	...
	public List<String> getViews(String viewName, PageBean pageBean)
		...
		return getForList(sql, pageBean, String.class,SqlTypeConst.DM);
	}
...
//		jdbcHelper.setCurrentDb(currentDb);
		...
		Map map = new HashMap();
		List<ColumnModel> list = jdbcHelper.queryForList(sql, map,new DmColumnMap());
...
//		jdbcHelper.setCurrentDb(currentDb);
		jdbcHelper=JdbcHelper.getInstance();
		List<ColumnModel> columnModels= jdbcHelper.queryForList(sql, new HashMap<String,Object>(), new DmColumnMap());
