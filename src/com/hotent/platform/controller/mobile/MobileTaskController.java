...
//					}
					...
					//form=bpmFormHandlerService.obtainHtml(bpmFormDef, userId, businessKey, "", actDefId, "", ctxPath);
				}
	...
	public Object getTaskForm(HttpServletRequest request,
		...
		try {
			...
			if(StringUtils.isNotEmpty(taskEntity.getDescription()) && taskEntity.getDescription().equals(TaskOpinion.STATUS_RESTART_TASK.toString())){
				...
				// if(isManage==0){
				// boolean hasRights = taskDao.getHasRightsByTask(new
				// Long(taskEntity.getId()), sysUser.getUserId());
				// if(!hasRights){
				...
				// 判断是否转办
				if (TaskOpinion.STATUS_RECOVER_TOSTART.toString().equals(
						taskEntity.getDescription())
						|| TaskOpinion.STATUS_REJECT_TOSTART.toString().equals(
								taskEntity.getDescription())
						|| TaskOpinion.STATUS_REJECT.toString().equals(
								taskEntity.getDescription())
						|| TaskOpinion.STATUS_DELEGATE.toString().equals(
								taskEntity.getDescription())) {
					isCanAssignee = false;
				}
				// 流程按钮
...
//		cmd.setUserAgent(userAgent);
	...
	public Object pendingMatters(HttpServletRequest request,
		...
		List<?> list = bpmService.getMyMobileTasks(filter);
	...
	//	filter.addFilter("status", BpmDefinition.STATUS_DEPLOYED);
		List<BpmDefinition> list = bpmDefinitionService.getList(filter, 0L);
