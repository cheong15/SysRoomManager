...
public class DictionaryController extends BaseController {
	...
	@RequestMapping("edit")
	@Action(description = "添加或编辑数据字典", execOrder = ActionExecOrder.AFTER, detail = "<#if isAdd==1>添加数据字典<#else>编辑数据字典"
	...
	public ModelAndView edit(HttpServletRequest request,
		...
		mv.addObject("dictionary", dictionary).addObject("isAdd", isAdd)
	...
	public void sort(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail, "字典排序失败:"
						+ str);
	...
	public Map<String, Object> getMapByNodeKey(HttpServletRequest request) {
		...
		ICache diccache = (ICache) AppUtil.getBean("iCache");
		if (diccache.containKey(nodeKey)) {
			return (Map<String, Object>) diccache.getByKey(nodeKey);
		} else {
			// 考虑加入缓存
			GlobalType globalType = globalTypeService.getByDictNodeKey(nodeKey);
			List<Dictionary> list = dictionaryService.getByTypeId(
					globalType.getTypeId(), false);
			map.put("globalType", globalType);
			map.put("dicList", list);
			diccache.add(nodeKey, map);
			return map;
		}
	}
	 ...
	 */
	@RequestMapping("move")
	@Action(description = "移动字典", detail = "<#assign dragEntity=dictionaryService.getById(Long.valueOf(dragId))/>"
	...
	public void move(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String str = MessageUtil.getMessage();
			if (StringUtil.isNotEmpty(str)) {
				resultMessage = new ResultMessage(ResultMessage.Fail, "移动字典失败:"
						+ str);
			} else {
