...
public class SysFileController extends BaseController {
	...
	public ModelAndView get(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//返回的Url地址
		String returnUrl = RequestUtil.getPrePage(request);
		return getAutoView().addObject("sysFile", sysFile).addObject("canReturn",canReturn).addObject("returnUrl",returnUrl);
	}
	...
	/*				if("pictureShow".equals(uploadType)){            //pictureShow控件要的文件路径要有一点不同
						sysFile.setFilePath(appUser.getAccount() +"/pictureShow" + "/" + year + "/" + month + "/" + fileName);
					}else{
						sysFile.setFilePath(appUser.getAccount() + "/" + year + "/" + month + "/" + fileName);
					}*/	
