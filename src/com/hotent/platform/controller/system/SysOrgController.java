...
public class SysOrgController extends BaseController {
	...
	public ModelAndView selector(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ModelAndView mv = this.getAutoView()
				.addObject("sysOrgList", sysOrgList).addObject("isSingle", isSingle);
		return mv;
	}
	...
	@Action(description = "删除组织",
			execOrder=ActionExecOrder.BEFORE,
			detail="删除组织" +
					"<#list StringUtils.split(orgId,\",\") as item>" +
					"<#assign entity=sysOrgService.getById(Long.valueOf(item))/>" +
					"【${entity.orgName}】"+
					"</#list>",
			exectype = "管理日志")
	...
	private void delByIds(Long[] lAryId){
		...
		for(Long id:lAryId){
			sysOrgService.delById(id);
			sysUserOrgService.delByOrgId(id);
			sysOrgRoleService.delByOrgId(id);
		}
	}
