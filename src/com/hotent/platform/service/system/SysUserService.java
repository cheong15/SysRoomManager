...
public class SysUserService extends  BaseService<SysUser>{
	...
	public List<SysUser> getByUserIdAndUplow(long userId, List<BpmNodeUserUplow> uplowList) {
		...
		for (BpmNodeUserUplow uplow : uplowList) {
			...
			if (uplow.getDemensionId().longValue() == Demension.positionDem.getDemId().longValue()) {
				...
				if(!demensionId.equals(ol.getDemId())) return null;
				if (ol != null) {
	...
	public void updPwdEncry(Long userId, String pwd) {
		//String enPassword = EncryptUtil.encryptSha256(pwd);
		dao.updPwd(userId, pwd);
	}
	 ...
	 * @throws Exception
	 */
	public void saveUser(Integer bySelf, SysUser sysUser, Long[] aryOrgIds, Long[] orgIdCharge, Long orgIdPrimary, Long[] posIds, Long posIdPrimary, Long[] roleIds)
			throws Exception {
	...
	public void syncToLdapAD(Long currentUserId, String dtForm, String dtTo) {
		dao.syncToLdapAD(currentUserId,dtForm,dtTo);
	}
