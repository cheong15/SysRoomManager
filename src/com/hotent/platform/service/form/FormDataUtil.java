...
public class FormDataUtil {
	...
	public static List<SqlModel> parseSql(BpmFormData formData,
		...
		Map<String, BpmFormTable> subTableMap = convertTableMap(mainTalble
				.getSubTableList());
		...
		if (isAdd) {
			...
			for (SubTable subTable : formData.getSubTableList()) {
				...
				for (Map<String, Object> row : dataList) {
					// 获取主键。
					String pk = row.get(subTable.getPkName().toLowerCase())
							.toString();
			...
			for (SubTable subTable : formData.getSubTableList()) {
				...
				BpmFormHandlerDao bpmFormHandlerDao = (BpmFormHandlerDao) AppUtil
						.getBean(BpmFormHandlerDao.class);
				// 原来的数据。
				List<Map<String, Object>> subDataList = bpmFormHandlerDao
						.getByFk(table, pkValue.getValue().toString(),
								actDefId, nodeId, true);
				List<Map<String, Object>> curDataList = subTable.getDataList();

				// 更新插入。
				List<SqlModel> updDelList = getUpdDelByList(table, curDataList,
						subDataList);
	 ...
	 * @return
	 */
	private static List<SqlModel> getUpdDelByList(BpmFormTable bpmFormTable,
		...
		Map<String, Map<String, Object>> originMap = convertMap(pkField,
				originList);
		Set<Entry<String, Map<String, Object>>> curSet = curMap.entrySet();
		// 遍历当前数据
		// 1.如果当前数据包含原来的数据，那么这个数据进行更新。
		// 2.如果当前数据不包含原来的数据，那么添加这个数据。
		for (Iterator<Entry<String, Map<String, Object>>> it = curSet
				.iterator(); it.hasNext();) {
		...
		Set<Entry<String, Map<String, Object>>> originSet = originMap
				.entrySet();
		for (Iterator<Entry<String, Map<String, Object>>> it = originSet
				.iterator(); it.hasNext();) {
			Entry<String, Map<String, Object>> ent = it.next();
			// 当前数据不包含之前的数据，则需要删除
			if (!curMap.containsKey(ent.getKey())) {
				String delSql = "delete from " + tableName + " where "
						+ pkField + "=?";
				SqlModel sqlModel = new SqlModel(delSql,
						new Object[] { ent.getKey() }, SqlModel.SQLTYPE_DEL);
	 ...
	 * @return
	 */
	private static Map<String, Map<String, Object>> convertMap(String pkField,
	 ...
	 * @throws Exception
	 */
	private static void handleMain(JSONObject jsonObj, BpmFormData bpmFormData)
			throws Exception {
		// 主表 main
		JSONObject mainTable = jsonObj.getJSONObject("main");
		BpmFormTable mainTableDef = bpmFormData.getBpmFormTable();
		String colPrefix = mainTableDef.isExtTable() ? ""
		...
		Map<String, Object> mainFiledsData = handleRow(mainTableDef,
				mainFieldTypeMap, mainFieldJson);
		// 添加主表数据
		...
		if (pkValue.getIsAdd()) {
			...
			// 通过脚本引擎计算字段。
			Map<String, Object> map = caculateField(colPrefix, mapFormField,
					bpmFormData.getMainFields());
	 ...
	 * @return
	 */
	private static Map<String, Object> getVariables(JSONObject jsonObject,
	 ...
	 * @throws Exception
	 */
	private static void handSub(JSONObject jsonObj, BpmFormData bpmFormData)
			throws Exception {
		...
		for (int i = 0; i < arySub.size(); i++) {
			...
			for (int j = 0; j < arySubFields.size(); j++) {
				...
				Map<String, Object> subRow = handleRow(subFormTable,
						subTableTypeMap, subFieldObj);
				// 计算脚本字段。
				Map<String, Object> map = caculateField(colPrefix,
						scriptFields, subRow);
	 ...
	 * @return
	 */
	private static List<BpmFormField> getFieldsFromScript(
			List<BpmFormField> list) {
	 ...
	 * @return
	 */
	private static Map<String, Object> caculateField(String colPrefix,
			List<BpmFormField> fields, Map<String, Object> rowData) {
	 ...
	 * @throws Exception
	 */
	public static PkValue generatePk(BpmFormTable bpmFormTable)
			throws Exception {
		Object pkValue = null;
		String pkField = bpmFormTable.getPkField().toLowerCase();
	 ...
	 * @param colPrefix
	 *            列前缀。
	 * @param subTableTypeMap
	 *            字段和类型映射。
	 * @param subFieldObj
	 *            json对象。
	 * @return
	 */
	private static Map<String, Object> handleRow(BpmFormTable bpmFormTable,
			Map<String, BpmFormField> fieldTypeMap, JSONObject fieldsJsonObj) {
		boolean isExternal = bpmFormTable.isExtTable();
	 ...
	 * @return
	 */
	private static Map<String, BpmFormField> convertFieldToMap(
			List<BpmFormField> list) {
	 ...
	 * @return
	 */
	private static Map<String, BpmFormTable> convertTableMap(
			List<BpmFormTable> list) {
	 ...
	 * @return
	 */
	private static SqlModel getInsert(BpmFormTable bpmFormTable,
			Map<String, Object> mapData) {
		String tableName = bpmFormTable.getFactTableName();
		String pkField = bpmFormTable.getPkField().toLowerCase();
		List<BpmFormField> flist = bpmFormTable.getFieldList();
		flist.get(0).getDatefmt();
		...
		for (Map.Entry<String, Object> entry : mapData.entrySet()) {
			...
			for (int i = 0; i < flist.size(); i++) {
				if (flist.get(i).getFieldName().toLowerCase().equals( entry.getKey())) {
					int ii=0;
					String fmt=flist.get(i).getDatefmt();
					if(flist.get(i).getFieldType().equals("date")){
						SimpleDateFormat sdf = new SimpleDateFormat(flist.get(i).getDatefmt());	
						Date s = null;
						try {
							s = sdf.parse(entry.getValue().toString());
	 ...
	 * @return
	 */
	private static SqlModel getUpd(String tableName, String pkField,
			Map<String, Object> mapData) {
		final List<Object> values = new ArrayList<Object>();
	...
	private static SqlModel getUpdate(BpmFormData bpmFormData) {
		...
		StringBuffer set = new StringBuffer();
		List<BpmFormField> flist = bpmFormData.getBpmFormTable().getFieldList();
	 ...
	 * @throws Exception
	 */
	public static BpmFormData parseJson(String json, BpmFormTable mainTableDef)
			throws Exception {
	...
	public static BpmFormData parseJson(String json, PkValue pkValue,
			BpmFormTable mainTableDef) throws Exception {
