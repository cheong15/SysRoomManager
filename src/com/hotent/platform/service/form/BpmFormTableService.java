...
public class BpmFormTableService extends BaseService<BpmFormTable> {
	...
	public int addFormTable(BpmFormTable table) throws Exception {
		...
		// 添加表
		table.setTableId(tableId);
		table.setDsAlias(SysDataSource.DS_LOCAL);
		table.setDsName(SysDataSource.DS_LOCAL);
...
//		}
	...
	public int upd(BpmFormTable table,int generate) throws Exception {
		...
		if (hasFormDef||hasData) {
			...
			//没有增加的列就直接返回。
			if (BeanUtils.isEmpty(addList))	return 0;
			int i = updList.size();
			int k = 0;
			// 添加字段
	...
	private BpmFormField addHiddenBpmFormField(BpmFormField field) {
		...
		// 设置人员选择器的ID脚本。
		if (valueFrom == BpmFormField.VALUE_FROM_SCRIPT_HIDDEN
				|| valueFrom == BpmFormField.VALUE_FROM_SCRIPT_SHOW) {
	...
	private List<BpmFormField> convertFields(List<BpmFormField> fields,
		...
		for (BpmFormField field : fields) {
			...
			if (isExecutorSelector(field.getControlType())
					&& field.getIsHidden().shortValue() == BpmFormField.NO_HIDDEN) {
	...
	public void generateTable(Long tableId, String operator) throws Exception {
		...
		// 将主表设为默认版本
		List<BpmFormField> mainFields = bpmFormFieldDao
		...
		for (BpmFormTable subTable : subTables) {
			...
			List<BpmFormField> subFields = bpmFormFieldDao
...
//			tableModel.addColumnModel(createTimeColumnModel);
	...
	private void getTabInfo(BpmFormTable bpmFormTable,int need){
		...
		if (bpmFormTable.getIsMain() == 0) return ;

		List<BpmFormTable> subTableList = dao.getSubTableByMainTableId(tableId);
		if (BeanUtils.isEmpty(subTableList)) return ;
	...
	public BpmFormTable getByDefId(Long defId) throws Exception{
		...
		if(bpmFormTable==null) return null;
	...
	private void updTable(BpmFormTable bpmFormTable, BpmFormTable orginTable)
		...
		for (int i = 0; i < updList.size(); i++) {
			...
			//更新选择器的隐藏字段    helh
			if (isExecutorSelector(bpmFormField.getControlType())
					&& bpmFormField.getIsHidden().shortValue() == BpmFormField.HIDDEN) {
				BpmFormField field = bpmFormFieldDao.getFieldByTidFna(bpmFormField.getTableId(), 
	...
	public void saveCopy(String json) throws Exception {
		...
		for (Object object : jsonArray) {
			...
			List<BpmFormField> fieldList = bpmFormFieldDao
	...
	public static boolean isExecutorSelector(Short controlType) {
		...
		if (controlType.shortValue() == FieldPool.SELECTOR_USER_SINGLE
				...
				|| controlType.shortValue() == FieldPool.SELECTOR_PROCESS_INSTANCE)
			return true;
		return false;
	}
	 ...
	 * @param identitySet
	 */
	public void setIdentity(List<Identity> identityList,
			Set<Identity> identitySet) {
	 ...
	 * @throws Exception
	 */
	public BpmFormTable convertBpmFormTableXml(BpmFormTableXml bpmFormTableXml,
		...
		if (BeanUtils.isNotEmpty(bpmFormTableXml.getBpmFormFieldList())) {
			List<BpmFormField> mainFields = bpmFormTableXml
					.getBpmFormFieldList();
		...
		if (isMain.shortValue() == BpmFormTable.NOT_MAIN.shortValue())
			return mainTable;
		List<BpmFormTableXml> subTableXmlList = bpmFormTableXml
				.getBpmFormTableXmlList();
		...
		for (BpmFormTableXml subTableXml : subTableXmlList) {
			BpmFormTable subTable = convertBpmFormTableXml(subTableXml,
					BpmFormTable.NOT_MAIN, map);
	 ...
	 * @throws Exception
	 */
	public Map<Long, Long> importBpmFormTableXml(BpmFormTableXml bpmFormTableXml)
			throws Exception {
		Short isMain = BpmFormTable.IS_MAIN;
		Map<Long, Long> map = new HashMap<Long, Long>();

		BpmFormTable bpmFormTable = this.convertBpmFormTableXml(
		...
		BpmFormTable destFormTable = this.convertBpmFormTableXml(
				bpmFormTableXml, isMain, map);

		if (destFormTable.getIsPublished().shortValue() == BpmFormTable.NOT_PUBLISH
				.shortValue()) {
	...
	public BpmFormTable getTableByIdContainHidden(Long tableId) {
		...
		bpmFormTable.setFieldList(fieldList);
		if (bpmFormTable.getIsMain().shortValue() == BpmFormTable.NOT_MAIN
				.shortValue())
			return bpmFormTable;
		...
		for (BpmFormTable table : subTableList) {
			List<BpmFormField> subFieldList = bpmFormFieldDao
	 ...
	 * @throws Exception
	 */
	private void importGenPhysicsTable(BpmFormTable destFormTable,
			BpmFormTable orginTable) throws Exception {
		if (BeanUtils.isEmpty(destFormTable))
			return;
		boolean isExists = bpmFormHandlerDao
	 ...
	 * @throws Exception
	 */
	private void importUpdTable(BpmFormTable destFormTable,
			BpmFormTable orginTable) throws Exception {
		boolean hasData = bpmFormHandlerDao
				.getHasData(TableModel.CUSTOMER_TABLE_PREFIX
						+ destFormTable.getTableName());
	 ...
	 * @throws Exception
	 */
	private void importDataTable(BpmFormTable destFormTable,
			BpmFormTable orginTable, Short isMain) throws Exception {
		...
		for (BpmFormTable subTable : subTableList) {
			...
			// 新加的子表。
				...
				TableModel subTableModel = this
						.getTableModelByBpmFormTable(subTable);
				tableOperator.createTable(subTableModel);
				tableOperator.addForeignKey(TableModel.CUSTOMER_TABLE_PREFIX
						+ tableName, TableModel.CUSTOMER_TABLE_PREFIX
						+ subTable.getTableName(), TableModel.PK_COLUMN_NAME,
	 ...
	 * @throws Exception
	 */
	public void importBpmFormTable(BpmFormTable bpmFormTable, Short isMain,
	 ...
	 * @throws Exception
	 */
	private BpmFormTable importFormTable(BpmFormTable bpmFormTable)
			throws Exception {
		BpmFormTable table = dao.getByTableName(bpmFormTable.getTableName());
		String isMain = (bpmFormTable.getIsMain().shortValue() == BpmFormTable.IS_MAIN) ? "主"
				: "从";
...
//			table.setUpdatetime(new Date());
	 ...
	 * @throws Exception
	 */
	private void importFormField(BpmFormField field, BpmFormTable bpmFormTable)
			throws Exception {
		BpmFormField bpmFormField = bpmFormFieldDao.getById(field.getFieldId());
		Long tableId = bpmFormTable.getTableId();
		if (BeanUtils.isEmpty(bpmFormField)) {
			BpmFormField bpmFormField1 = bpmFormFieldDao.getFieldByTidFna(
...
//			field.setCreatetime(new Date());
	...
	private void importIdentity(Identity identity) throws Exception {
		...
		if (isExist == null) {
			...
			MsgUtil.addMsg(MsgUtil.SUCCESS, "流水号为‘" + identity.getName()
					+ "’，该流水号成功导入！");
		} else {
			MsgUtil.addMsg(MsgUtil.ERROR, "流水号为‘" + identity.getName()
					+ "’已经存在，该流水号终止导入！");
		}
	}
	...
	private void exportSubTable(Long tableId, Map<String, Boolean> map,
			List<BpmFormTableXml> bpmFormTableXmlList) {
		List<BpmFormTable> subTables = dao.getSubTableByMainTableId(tableId);
