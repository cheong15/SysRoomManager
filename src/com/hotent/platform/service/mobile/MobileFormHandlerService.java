...
public class MobileFormHandlerService extends
	...
	public MobileFormData getMobileForm(MobileFormData mobileFormData,
		...
		if (bpmNodeSet.getFormKey() == null
				|| bpmNodeSet.getFormKey().longValue() == 0) {
			isEmptyForm = true;
		}
		...
		if (!isExtForm) {
			...
			mobileFormData = this.getMobileFormData(bpmFormDef, pkValue,
					processRun, mobileFormData,userId,nodeId);
		}
	...
	public MobileFormData getMobileFormData(BpmFormDef bpmFormDef,
			String pkValue, ProcessRun processRun, MobileFormData mobileFormData,Long userId, String nodeId)
			throws Exception {
		...
		//表单中的当前可以用的意见
		Map<String, Map<String, String>> permission = bpmFormRightsService.getByFormKeyAndUserId(bpmFormDef.getFormKey(), userId,processRun.getActDefId(), nodeId);
		return this.getFormData(table, pkValue, processRun.getActInstId(), tableMap,permission);
	}
	 ...
	 * @return
	 */
	private String parseFile(String value) {
		...
		try {
			...
			for (int i = 0; i < jsonAry.size(); i++) {
				...
				fileValue.append(MobileFormHandlerService.getFile(id, name));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public static String getFile(String id, String name) {
		String path = "platform/system/sysFile/file_" + id
				+ ".ht?isDownload=true";
		String file = name + "," + name;
		return "<span class='attachement-span'><span fileId='" + id
				+ "' name='attach' file='" + file
				+ "' ><a class='attachment' target='_blank' href='" + path
				+ "' title='" + name + "'>" + name + "</a></span>&nbsp;</span>";
	}

	// TODO ============= 以下是起始表单的数据 =====================

	public MobileFormData getMobileFormData(BpmFormDef bpmFormDef,
			String pkValue, MobileFormData mobileFormData) throws Exception {
		if (BeanUtils.isEmpty(bpmFormDef))
			return mobileFormData;
		...
		BpmFormTable table = bpmFormTableDao.getById(tableId);
		Map<String, String> tableMap = this.parseTableMap(bpmFormDef, table);
		return this.getFormData(tableId, pkValue, tableMap);
	}

	private MobileFormData getFormData(Long tableId, String pkValue,
			Map<String, String> tableMap) throws Exception {
		// TODO Auto-generated method stub
		...
		// 取出表单数据
		BpmFormData bpmFormData = bpmFormHandlerService.getBpmFormData(tableId,
				pkValue, null, null, null);

		formData.setTableId(table.getTableId());
		formData.setTableName(table.getTableDesc());

		// 设置主表字段数据
		this.setMainData(formData, bpmFormData, tableMap, table);
		// 设置意见
		formData.setOptions(bpmFormData.getOptions());
		// 设置子表的数据
		this.setSubDataList(formData, bpmFormData, tableMap, tableId);
	...
	private void setMainData(MobileFormData formData, BpmFormData bpmFormData,
			Map<String, String> tableMap, BpmFormTable table) {
		// 根据主表ID取出主表字段列表
		List<BpmFormField> mainFieldList = bpmFormFieldService
				.getAllByTableId(table.getTableId());
		JSONArray fieldJson = new JSONArray();
